...
var ViewModel = function(map) {
    ...
    initialAnimals.forEach(function(animalItem) {
        ...
        //the marker event listener is added here, because ViewModel knows the infoboxmodel, while map does not
        marker.addListener('click', function() {
            self.ibm.setSelectedAnimal(marker.animal);
        });
        //also, I wanted to have everything together, therefore the marker is saved in the animalitem. ibm can then handle active of the marker.
    ...
    //here, some more or less rather ugly queries must be made in order to set the variables according to the
    ...
    //on each window resize, the visibilites need to be set new, so the knockout bindings get updated correctly, and it is
